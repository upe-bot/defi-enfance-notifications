...
//  BREVO
// ══════════════════════════════════════════════════════
async function sendBrevo(to, subject, html, tag = null) {
  ...
  try {
    const payload = { sender: { name: CONFIG.senderName, email: CONFIG.senderEmail }, to: [{ email: toClean }], subject, htmlContent: html };
...
const REDIS_KEY_CAMPAGNES_PERSO = 'campagnes_perso_v1';

// Slugifie un nom de campagne en tag Brevo valide
function slugifierTag(nom) {
  return (nom || 'campagne')
...
async function chargerCampagnesPerso() {
  try {
    const raw = await redisGet(REDIS_KEY_CAMPAGNES_PERSO);
    if (raw) { const p = JSON.parse(raw); return Array.isArray(p) ? p : []; }
  } catch(e) {}
  return [];
}
async function sauverCampagnesPerso(liste) {
  try { await redisSet(REDIS_KEY_CAMPAGNES_PERSO, JSON.stringify(liste)); } catch(e) {}
}

// État de l'envoi emailing perso
const emailingPerso = {
  running: false, total: 0, done: 0, sent: 0, errors: 0, skipped: 0,
  startedAt: null, finishedAt: null, log: [], campagneNom: null, tag: null,
};
...
app.post('/api/emailing-perso/start', async (req, res) => {
  if (emailingPerso.running) return res.json({ error: 'Un envoi est déjà en cours' });
  const { lignes, htmlTemplate, sujetTemplate, campagneNom } = req.body || {};
  ...
  if (!campagneNom || !campagneNom.trim()) return res.json({ error: 'Nom de campagne manquant' });

  const dateEnvoi = new Date();
  const dateStr = dateEnvoi.toISOString().slice(0, 10);
  const tag = `${slugifierTag(campagneNom)}-${dateStr}`;
  ...
  emailingPerso.startedAt = dateEnvoi.toISOString();
  emailingPerso.finishedAt = null;
  emailingPerso.log = [];
  emailingPerso.campagneNom = campagneNom.trim();
  emailingPerso.tag = tag;
  res.json({ success: true, total: lignes.length, tag });

  (async () => {
    emailingPersoLog(`📨 Démarrage campagne "${campagneNom}" (tag: ${tag}) — ${lignes.length} destinataire(s)`, 'info');
    const emailsVus = new Set();
    for (const ligne of lignes) {
      ...
      const ok = await sendBrevo(email, sujet, html, tag);
    ...
    // Enregistrer la campagne dans Redis
    try {
      const campagnes = await chargerCampagnesPerso();
      campagnes.unshift({
        nom: campagneNom.trim(), tag, dateEnvoi: dateEnvoi.toISOString(),
        sujet: sujetTemplate, total: lignes.length,
        envoyes: emailingPerso.sent, erreurs: emailingPerso.errors, ignores: emailingPerso.skipped,
      });
      await sauverCampagnesPerso(campagnes.slice(0, 100));
      emailingPersoLog(`💾 Campagne "${campagneNom}" enregistrée`, 'ok');
    } catch(e) { emailingPersoLog(`⚠️ Sauvegarde campagne : ${e.message}`, 'warn'); }
  })().catch(e => {
    emailingPerso.running = false;
    emailingPerso.finishedAt = new Date().toISOString();
...
app.get('/api/emailing-perso/status', (req, res) => {
  res.json({
    ...
    campagneNom: emailingPerso.campagneNom, tag: emailingPerso.tag,
    log: emailingPerso.log.slice(0, 200),
  });
});

// LISTE des campagnes enregistrées
app.get('/api/emailing-perso/campagnes', async (req, res) => {
  const campagnes = await chargerCampagnesPerso();
  res.json(campagnes);
});

// REPORTING Brevo pour une campagne (par tag)
app.get('/api/emailing-perso/campagnes/:tag/stats', async (req, res) => {
  const tag = req.params.tag;
  if (!CONFIG.brevoKey) return res.json({ error: 'Clé Brevo manquante' });
  try {
    const campagnes = await chargerCampagnesPerso();
    const campagne = campagnes.find(c => c.tag === tag);
    const startDate = campagne ? campagne.dateEnvoi.slice(0, 10) : null;
    const endDate = new Date().toISOString().slice(0, 10);

    let url = `https://api.brevo.com/v3/smtp/statistics/aggregatedReport?tag=${encodeURIComponent(tag)}`;
    if (startDate) url += `&startDate=${startDate}&endDate=${endDate}`;
    const r = await fetch(url, { headers: { 'accept': 'application/json', 'api-key': CONFIG.brevoKey } });
    if (!r.ok) {
      const err = await r.json().catch(() => ({}));
      return res.json({ error: `Brevo ${r.status}: ${err.message || 'erreur'}` });
    }
    ...
    res.json({
      tag, brut: stats,
      calcul: {
        envois:            stats.requests || 0,
        delivres:          delivered,
        ouvertures:        stats.opens || 0,
        ouverturesUniques: stats.uniqueOpens || 0,
        clics:             stats.clicks || 0,
        clicsUniques:      stats.uniqueClicks || 0,
        bounces:           (stats.hardBounces || 0) + (stats.softBounces || 0),
        spam:              stats.spamReports || 0,
        desabo:            stats.unsubscribed || 0,
        tauxOuverture:     delivered > 0 ? Math.round((stats.uniqueOpens || 0) / delivered * 1000) / 10 : 0,
        tauxClic:          delivered > 0 ? Math.round((stats.uniqueClicks || 0) / delivered * 1000) / 10 : 0,
        tauxLivraison:     (stats.requests || 0) > 0 ? Math.round(delivered / stats.requests * 1000) / 10 : 0,
      }
    });
  } catch(e) {
    res.json({ error: e.message });
